...
# ==========================================
# CUSTOM CSS  — Deep Sapphire Glassmorphism
# Tema: Navy Indigo + Teal Accent + Amber Highlight
# Revisi: Modern, Elegan, Non-monoton
# ==========================================
st.markdown("""
<style>
/* ── 1. GOOGLE FONTS ─────────────────────────────────────────── */
...
:root {
    /* Background Layers — deep navy, bukan hitam pekat */
    --bg-primary:     #07101f;          /* halaman utama: navy sangat gelap */
    --bg-surface:     #0d1b30;          /* sidebar & panel */
    --bg-card:        #122040;          /* kartu & form */
    --bg-hover:       #182c52;          /* hover state */
    --bg-glass:       rgba(13,27,48,.7);/* glassmorphism overlay */

    /* Border */
    --border:         #1e3a60;          /* border normal */
    --border-light:   #152b4a;          /* border sangat tipis */
    --border-accent:  rgba(0,210,180,.3);

    /* Accent Utama: Teal-Mint → segar, kesehatan, gizi */
    --accent:         #00d4b4;
    --accent-dim:     #009e87;
    --accent-glow:    rgba(0,212,180,.15);
    --accent-hover:   #00ffe0;

    /* Accent Sekunder: Amber → ranking, pilihan, highlight */
    --accent2:        #f59e0b;
    --accent2-dim:    #b97a08;
    --accent2-glow:   rgba(245,158,11,.15);

    /* Status Colors */
    --info:           #60a5fa;          /* biru cornflower */
    --info-glow:      rgba(96,165,250,.15);
    --warn:           #fbbf24;
    --danger:         #f87171;
    --success:        #34d399;
    ...
    --shadow-card:   0 2px 16px rgba(0,0,0,.4), 0 0 0 1px rgba(0,210,180,.06);
    --shadow-glow:   0 0 24px rgba(0,212,180,.18);
}
...
/* Subtle mesh gradient background untuk depth */
.stApp {
    background:
        radial-gradient(ellipse 80% 50% at 20% 10%, rgba(0,150,130,.07) 0%, transparent 60%),
        radial-gradient(ellipse 60% 40% at 80% 80%, rgba(96,165,250,.05) 0%, transparent 50%),
...
section[data-testid="stSidebar"] > div { padding: 0 0.85rem; }

/* ── 5a. TOMBOL NAVIGASI SIDEBAR (pill + left-bar aktif) ────── */
...
section[data-testid="stSidebar"] .stButton > button:hover {
    background: rgba(0,212,180,.08) !important;
    border-left-color: var(--accent-dim) !important;
    color: var(--text-primary) !important;
    padding-left: 1.15rem !important;
}
/* Tombol aktif di sidebar — primary kind */
section[data-testid="stSidebar"] .stButton > button[kind="primary"] {
    ...
    padding-left: 1.15rem !important;
    box-shadow: inset 0 0 24px rgba(0,212,180,.05) !important;
}
section[data-testid="stSidebar"] .stButton > button[kind="primary"]:hover {
    background: rgba(0,212,180,.18) !important;
    color: var(--accent-hover) !important;
}

/* ── 6. TOMBOL KONTEN UTAMA ─────────────────────────────────── */

/* Primary — aksi utama (Hitung, Simpan, Konfirmasi) */
.stButton > button[kind="primary"],
.stButton > button:not([kind="secondary"]) {
    background: linear-gradient(135deg, var(--accent-dim) 0%, var(--accent) 100%) !important;
    border: 1px solid var(--accent) !important;
    color: #031a14 !important;          /* teks gelap agar kontras di teal */
    font-weight: 700 !important;
...
.stButton > button:not([kind="secondary"]):hover {
    ...
    box-shadow: 0 4px 20px var(--accent-glow), 0 0 0 1px var(--accent) !important;
    transform: translateY(-2px) !important;
}

/* Secondary — aksi pendukung (Batal, Cari, Logout) */
.stButton > button[kind="secondary"] {
    background: rgba(30,58,96,.5) !important;
...
.stButton > button[kind="secondary"]:hover {
    ...
    background: var(--info-glow) !important;
...
.stButton > button[disabled] {
    background: rgba(30,58,96,.3) !important;
...
.stNumberInput input:focus {
    border-color: var(--accent) !important;
    box-shadow: 0 0 0 3px var(--accent-glow), 0 2px 8px rgba(0,0,0,.3) !important;
    outline: none !important;
    background: rgba(18,32,64,1) !important;
}
/* Label universal — konsisten */
.stTextInput label,
.stTextArea label,
.stNumberInput label,
.stSelectbox label,
...
/* ── 8. FORM CONTAINER ──────────────────────────────────────── */
.stForm {
    background: rgba(18,32,64,.6) !important;
    border: 1px solid var(--border) !important;
...
[data-baseweb="select"] > div {
    background: rgba(18,32,64,.85) !important;
...
/* ── 11. TABEL KUSTOM (HTML) ────────────────────────────────── */
/*
 * Karena st.dataframe merender di dalam iframe Streamlit (shadow DOM),
 * CSS eksternal tidak bisa menembus ke dalam tabelnya.
 * Solusi: gunakan HTML table kustom yang kita render via st.markdown()
 * sehingga kita punya kendali penuh atas warna, padding, dan tipografi.
 */

/* ── Wrapper tabel — border, radius, shadow, backdrop */
.mbg-table-wrapper {
...
/* ── Scrollable body jika konten panjang secara horizontal */
.mbg-table-scroll {
    overflow-x: auto;
    width: 100%;
}
...
/* ── Header — sticky, high-contrast, teal */
.mbg-table thead tr {
    background: linear-gradient(
        135deg,
        rgba(0,212,180,.14) 0%,
        rgba(0,158,135,.08) 100%
    );
    position: sticky;
    top: 0;
    z-index: 2;
...
.mbg-table thead th {
    ...
    letter-spacing: .1em;
    color: var(--accent);
    border-bottom: 2px solid rgba(0,212,180,.35);
    white-space: nowrap;
    line-height: 1.4;
    /* Alignment default: semua header center */
    text-align: left;
}
/* Header kolom angka — rata kanan */
.mbg-table thead th.col-number,
.mbg-table thead th.col-score,
.mbg-table thead th.col-badge {
    text-align: right;
}
.mbg-table thead th.col-badge { text-align: center; }

/* ── Baris data */
.mbg-table tbody tr {
    border-bottom: 1px solid rgba(30,58,96,.55);
    transition: background .15s ease;
}

/* Zebra striping — genap sedikit lebih terang */
.mbg-table tbody tr:nth-child(even) {
    background: rgba(13,27,48,.85);
}
.mbg-table tbody tr:nth-child(odd) {
    background: rgba(7,16,31,.9);
}

/* Baris top-1 (class khusus) */
.mbg-table tbody tr.row-top1 {
    background: rgba(0,212,180,.06) !important;
    border-left: 3px solid var(--accent);
}
.mbg-table tbody tr.row-top2 {
    background: rgba(148,163,184,.03) !important;
}
.mbg-table tbody tr.row-top3 {
    background: rgba(180,120,80,.04) !important;
}
...
/* ── Tipe kolom: angka → kanan, monospace, teal dim */
.mbg-table td.col-number {
    text-align: right;
    font-family: 'JetBrains Mono', monospace;
    font-size: .82rem;
    color: #9ec8fa;           /* biru muda — angka gizi */
    letter-spacing: .01em;
}

/* ── Tipe kolom: skor WP → kanan, teal bright, monospace kecil */
.mbg-table td.col-score {
    text-align: right;
    font-family: 'JetBrains Mono', monospace;
    font-size: .78rem;
    color: var(--accent);
...
/* ── Tipe kolom: badge/rank → center */
.mbg-table td.col-badge {
    text-align: center;
    font-family: 'JetBrains Mono', monospace;
    font-weight: 700;
    font-size: .82rem;
...
/* ── Tipe kolom: code/kode → center, pill */
.mbg-table td.col-code {
    text-align: center;
}
.mbg-table td.col-code span {
...
/* ── Kolom teks panjang — wrap normal */
.mbg-table td.col-text {
    max-width: 340px;
    white-space: normal;
...
/* ── Kolom teks pendek — nowrap */
.mbg-table td.col-text-sm {
    white-space: nowrap;
    color: var(--text-secondary);
    font-size: .84rem;
}

/* ── Rank badge di dalam sel */
.rank-pill {
    display: inline-flex;
    align-items: center;
...
.rank-pill-1 { background: rgba(245,158,11,.2); color: #fbbf24; border: 1px solid rgba(245,158,11,.45); }
.rank-pill-2 { background: rgba(148,163,184,.18); color: #94a3b8; border: 1px solid rgba(148,163,184,.4); }
.rank-pill-3 { background: rgba(180,120,80,.2); color: #cd9b6a; border: 1px solid rgba(180,120,80,.45); }
.rank-pill-n { background: rgba(30,58,96,.6); color: var(--text-muted); border: 1px solid var(--border); }

/* ── Pagination bar di atas tabel */
.tabel-pagination {
    display: flex;
    align-items: center;
    gap: .8rem;
...
/* ── Sel kosong / null */
.mbg-table td.col-empty {
    color: var(--text-muted);
    font-style: italic;
    font-size: .82rem;
}

/* ── Baris kosong (no data) */
.mbg-table-empty {
    padding: 2rem 1.5rem;
    text-align: center;
    color: var(--text-muted);
...
/* ── 13. TABS ───────────────────────────────────────────────── */
.stTabs [role="tablist"] {
    background: rgba(13,27,48,.7) !important;
    border-radius: var(--radius-md) !important;
...
.streamlit-expanderHeader {
    background: rgba(13,27,48,.8) !important;
    border: 1px solid var(--border) !important;
    border-radius: var(--radius-sm) !important;
    color: var(--text-primary) !important;
    font-weight: 600 !important;
...
.streamlit-expanderContent {
    background: rgba(18,32,64,.5) !important;
    border: 1px solid var(--border) !important;
...
h1, h2, h3, h4 { color: var(--text-primary) !important; font-weight: 800 !important; }

/* ── 16. KOMPONEN KUSTOM — LOGIN ────────────────────────────── */
.login-wrapper {
    min-height: 85vh;
    display: flex;
...
.login-hero h1 {
    ...
    line-height: 1.2;
    /* Gradient teal → biru cerah, non-monoton */
    background: linear-gradient(135deg, var(--accent) 0%, #60a5fa 60%, #a78bfa 100%);
...
.login-card:hover {
    ...
    box-shadow: 0 12px 40px rgba(0,0,0,.6), 0 0 0 1px var(--border-accent);
    border-color: rgba(0,210,180,.2);
}
.login-card-header {
...
.icon-sppg { background: rgba(0,212,180,.15); border: 1px solid rgba(0,212,180,.25); }
.icon-gizi { background: rgba(96,165,250,.15); border: 1px solid rgba(96,165,250,.25); }
.login-card-title {
    font-size: 1rem !important; font-weight: 700 !important;
    color: var(--text-primary) !important; margin: 0 !important;
}
.login-card-subtitle { font-size: .76rem; color: var(--text-muted); }
...
/* Garis atas berwarna gradient — bukan hitam monoton */
.stat-card::before {
    content: '';
    position: absolute; top: 0; left: 0; right: 0; height: 3px;
    background: linear-gradient(90deg, var(--accent), var(--info), var(--accent2));
}
.stat-label {
    font-size: .73rem; font-weight: 700; text-transform: uppercase;
    letter-spacing: .09em; color: var(--text-muted); margin-bottom: .4rem;
}
.stat-value {
...
/* ── 20. WINNER CARD (Top Ranking) ──────────────────────────── */
.winner-card {
    /* Gradient teal ke navy — jelas berbeda dari bg utama */
    background: linear-gradient(135deg,
        rgba(0,158,135,.12) 0%,
        rgba(18,32,64,.95) 100%
    );
    ...
    margin: 1rem 0;
    position: relative; overflow: hidden;
    box-shadow: 0 4px 24px rgba(0,212,180,.1);
}
.winner-card::before {
    content: '🏆';
    position: absolute; right: 1.5rem; top: 50%;
    transform: translateY(-50%);
    font-size: 3.5rem; opacity: .1;
}
.winner-label {
...
.winner-score {
    ...
    border: 1px solid rgba(0,212,180,.3);
    padding: .2rem .75rem; border-radius: 20px; margin-top: .6rem;
}

/* ── 21. SELECTED CARD (Menu Terpilih) ──────────────────────── */
.selected-card {
    /* Gradient amber ke navy — berbeda dari winner-card */
    background: linear-gradient(135deg,
        rgba(245,158,11,.1) 0%,
        rgba(18,32,64,.95) 100%
    );
...
.selected-label {
    font-size: .72rem; font-weight: 700; text-transform: uppercase;
    letter-spacing: .1em; color: var(--accent2); margin-bottom: .5rem;
}
.selected-score {
    display: inline-block;
    font-family: 'JetBrains Mono', monospace; font-size: .85rem;
    background: var(--accent2-glow); color: var(--accent2);
    border: 1px solid rgba(245,158,11,.3);
    padding: .2rem .75rem; border-radius: 20px; margin-top: .6rem;
}
...
/* Emas */
.rank-1 { background: rgba(245,158,11,.2); color: #fbbf24; border: 1px solid rgba(245,158,11,.45); }
/* Perak */
.rank-2 { background: rgba(148,163,184,.15); color: #94a3b8; border: 1px solid rgba(148,163,184,.35); }
/* Perunggu */
.rank-3 { background: rgba(180,120,80,.18); color: #cd9b6a; border: 1px solid rgba(180,120,80,.4); }
/* Lainnya */
.rank-n { background: var(--bg-hover); color: var(--text-muted); border: 1px solid var(--border); }

/* ── 23. RANK CARD (List Perangkingan) ──────────────────────── */
.rank-card {
    background: var(--bg-glass);
    border: 1px solid var(--border);
    border-radius: var(--radius-md);
...
/* Rank #1 — lebih menonjol */
.rank-card-top {
    border-color: rgba(0,212,180,.4);
    background: rgba(0,212,180,.06);
    box-shadow: 0 2px 16px rgba(0,212,180,.08);
}
...
.section-title {
    ...
    letter-spacing: .12em;
    /* Gradient teks — lebih hidup dari solid muted */
    color: var(--text-secondary);
...
/* ── 25. INFO BOX & WARN BOX ────────────────────────────────── */
/* Palet lebih kaya — bukan sekadar hitam semi-transparan */
.info-box {
...
.user-card-name { font-weight: 700; font-size: .93rem; color: var(--text-primary); }
.user-card-meta { font-size: .76rem; color: var(--text-muted); margin-top: .15rem; }
.user-card-badge { margin-top: .55rem; }

/* ── 27. NAV LABEL ──────────────────────────────────────────── */
.nav-label {
    font-size: .68rem; font-weight: 800; text-transform: uppercase;
    letter-spacing: .12em; color: var(--text-muted);
    margin: 1.1rem 0 .35rem; padding-left: .5rem;
    /* Subtle separator */
...
# Menghasilkan tabel HTML yang sepenuhnya kita kendalikan
# sehingga tema navy glassmorphism benar-benar teraplikasi.
# ──────────────────────────────────────────────────────────────

def _format_cell(value, col_type: str, col_name: str = ""):
    """
    Kembalikan (html_content, css_class) untuk satu sel.
    col_type: 'text' | 'text_sm' | 'number' | 'score' | 'badge' | 'code' | 'rank'
    """
    if value is None or (isinstance(value, float) and str(value) == 'nan'):
        return '<span style="color:var(--text-muted);font-style:italic;">—</span>', "col-empty"

    val_str = str(value)

    if col_type == "rank":
        n = int(value) if str(value).isdigit() else 0
        pill_cls = f"rank-pill-{n}" if n <= 3 else "rank-pill-n"
        return f'<span class="rank-pill {pill_cls}">#{n}</span>', "col-badge"

    if col_type == "code":
        return f'<span>{val_str}</span>', "col-code"

    if col_type == "score":
        try:
            num = float(value)
            return f'{num:.8f}', "col-score"
        except (ValueError, TypeError):
            return val_str, "col-score"
    ...
    if col_type == "number":
        try:
            ...
            if num == int(num) and "." not in str(value):
                return f'{int(num):,}', "col-number"
            return f'{num:.1f}', "col-number"
        except (ValueError, TypeError):
            return val_str, "col-number"

    if col_type == "number_int":
        try:
            return f'{int(value):,}', "col-number"
        except (ValueError, TypeError):
            return val_str, "col-number"
    ...
    if col_type == "bobot":
        try:
            num = float(value)
            return f'{num:.2f}', "col-number"
        except (ValueError, TypeError):
            return val_str, "col-number"

    if col_type == "text_sm":
        return val_str, "col-text-sm"

    # Default: text biasa
    return val_str, "col-text"
...
def render_html_table(
    ...
    """
    Render DataFrame sebagai HTML table bertema navy glassmorphism.

    Parameters
    ----------
    df            : DataFrame yang akan dirender
    col_types     : dict {nama_kolom: tipe}
                    Tipe: 'text' | 'text_sm' | 'number' | 'number_int' |
                           'bobot' | 'score' | 'badge' | 'code' | 'rank'
    max_height_px : Jika diisi, tabel bisa di-scroll vertikal
    highlight_top_n: Warnai N baris pertama sebagai top-ranking
    """
    ...
    for col in df.columns:
        ...
        align_cls = "col-number" if ctype in ("number", "number_int", "score", "bobot") else (
                    "col-badge"  if ctype in ("rank", "badge") else "")
        th_rows += f'<th class="{align_cls}">{col}</th>'

    # ── Body rows
    tbody = ""
    for i, (_, row) in enumerate(df.iterrows()):
        # Kelas baris untuk highlight ranking teratas
        if highlight_top_n > 0 and i < highlight_top_n:
            row_cls = f"row-top{i+1}" if i < 3 else ""
        else:
            row_cls = ""
        ...
        for col in df.columns:
            ctype = col_types.get(col, "text")
            content, td_cls = _format_cell(row[col], ctype, col)
            tds += f'<td class="{td_cls}">{content}</td>'

        tbody += f'<tr class="{row_cls}">{tds}</tr>'

    if not tbody:
        tbody = f'<tr><td colspan="{len(df.columns)}" class="mbg-table-empty">ℹ️ Tidak ada data.</td></tr>'

    # ── Scroll wrapper opsional
    scroll_style = f'max-height:{max_height_px}px;overflow-y:auto;' if max_height_px else ''

    html = f"""
<div class="mbg-table-wrapper">
  <div class="mbg-table-scroll" style="{scroll_style}">
    <table class="mbg-table">
...
def tampilkan_tabel_dengan_paginasi(
    df,
    baris_per_halaman: int = 10,
    key_prefix: str = "tabel",
    col_types: dict = None,
    highlight_top_n: int = 0,
    max_height_px: int = None,
    # Parameter lama (config) diabaikan — digantikan col_types
    config=None,
):
    """
    Tampilkan DataFrame dengan paginasi + HTML table bertema navy glassmorphism.

    Parameters
    ----------
    df               : DataFrame sumber
    baris_per_halaman: Jumlah baris per halaman (default 10)
    key_prefix       : Prefix unik untuk widget paginasi
    col_types        : dict tipe kolom → lihat render_html_table()
    highlight_top_n  : Warnai N baris pertama sebagai top ranking
    max_height_px    : Batasi tinggi tabel (px) lalu bisa di-scroll
    config           : (diabaikan, untuk kompatibilitas lama)
    """
    import pandas as _pd

    total_baris = len(df)
    if total_baris == 0:
        st.markdown('<div class="info-box">ℹ️ Belum ada data untuk ditampilkan.</div>',
                    unsafe_allow_html=True)
        return

    total_halaman = max(1, (total_baris - 1) // baris_per_halaman + 1)

    # ── Baris kontrol paginasi ────────────────────────────────
    col_chip, col_nav, col_rest = st.columns([3, 2, 5])
    with col_chip:
        st.markdown(
            f'<div class="tabel-pagination">'
            f'<span class="tabel-count-chip">📊 {total_baris} data</span>'
            f'</div>',
            unsafe_allow_html=True
        )
    if total_halaman > 1:
        with col_nav:
            halaman_aktif = st.number_input(
                f"Hal. (maks. {total_halaman})",
                min_value=1, max_value=total_halaman,
                step=1, key=f"{key_prefix}_page",
                label_visibility="collapsed",
            )
            st.markdown(
                f'<div class="tabel-page-info" style="margin-top:-.4rem;">'
                f'Hal. <b style="color:var(--text-secondary);">{int(halaman_aktif)}</b>'
                f' / {total_halaman}</div>',
                unsafe_allow_html=True
            )
    else:
    ...
    mulai = (int(halaman_aktif) - 1) * baris_per_halaman
    akhir = mulai + baris_per_halaman
    df_page = df.iloc[mulai:akhir].reset_index(drop=True)
        ...
        with col_tabel:
            ...
            if data_bahan:
                ...
                tampilkan_tabel_dengan_paginasi(
                    df_tampil, baris_per_halaman=5, key_prefix="bahan_baku",
                    col_types={
                        "No":          "number_int",
                        "Nama Bahan":  "text",
                        "Waktu Input": "text_sm",
            ...
            for i, row in top5.iterrows():
                ...
                <div class="rank-card {'rank-card-top' if peringkat == 1 else ''}">
                    ...
                    <div style="font-family:'JetBrains Mono',monospace;font-size:.82rem;color:var(--accent);
                                background:var(--accent-glow);padding:.22rem .7rem;border-radius:14px;
                                border:1px solid var(--border-accent);white-space:nowrap;">
                        {skor:.8f}
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
def dasbor_ahli_gizi():
    ...
    if st.session_state.menu_aktif_gizi == "Rancang Menu":
        ...
        with st.expander("📋  Lihat bahan baku tersedia (dari SPPG)", expanded=False):
            ...
            if bahan_tersedia:
                tags_html = " ".join([
                    f'<span style="background:var(--accent-glow);border:1px solid var(--border-accent);'
                    f'color:var(--accent);padding:.2rem .65rem;border-radius:20px;font-size:.8rem;margin:.2rem;'
    ...
    # ── FITUR 2: BOBOT KRITERIA ──────────────────────────────────────
        ...
        if not data_kriteria:
            ...
            with st.form("form_edit_bobot_massal"):
                ...
                box_style = "background:var(--accent-glow);border-color:var(--border-accent);color:var(--accent);" if is_valid else ""
                st.markdown(
    ...
    # ── FITUR 3: PERANGKINGAN WP ─────────────────────────────────────
        ...
        with col_s1:
            warna = "var(--accent)" if jumlah_menu >= 5 else "var(--warn)"
            st.markdown(f"""
            <div class="stat-card">
        ...
        with col_s2:
            warna2 = "var(--accent)" if jumlah_dinilai == jumlah_menu and jumlah_menu >= 5 else "var(--warn)"
            st.markdown(f"""
            <div class="stat-card">
        ...
        with col_s3:
            ...
            warna3 = "var(--accent)" if status_siap else "var(--warn)"
            st.markdown(f"""
            <div class="stat-card">
        ...
        if st.session_state.hasil_wp:
            ...
            if st.session_state.menu_terpilih_kode:
                ...
                st.markdown(
                    f'<div class="info-box" style="background:var(--accent2-glow);border-color:rgba(245,158,11,.3);color:var(--accent2);margin-top:1rem;">'
